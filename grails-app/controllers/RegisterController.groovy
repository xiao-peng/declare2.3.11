...
class RegisterController {
    ...
    def online(){
        ...
        if(BaseUser.countByUsername(params.email)>0){
            ...
            def bu=new BaseUser(username: params.email,enabled: false,password: params.password,firstName:' ',lastName: ' ',email:params.email,phone:params.phone,type: params.type);
            bu.save(flush: true);
            if(bu.errors.allErrors.size()==0){
                BaseUserBaseRole.create(bu,BaseRole.findByAuthority('ROLE_MEMBER'),true);
                map.result=true;
                map.url="http://mail."+params.email.tokenize('@')[1];
                String path = request.getContextPath();
                String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
                def url = "${basePath}register/active?token="+bu.id;
                def timer=new Timer();
