...
class BootStrap {
    ...
    def createDefaultUsers() {
        def superadmin = BaseUser.findByUsername('superadmin')
        if (!superadmin) {
            superadmin = new BaseUser(username:'superadmin',password: 'superadmin',enabled:true,accountExpired:false,accountLocked:false,passwordExpired:false)
        ...
        new BaseUserBaseRole(baseUser:superadmin,baseRole:BaseRole.findByAuthority('ROLE_SUPERADMIN')).save(flush: true);
    ...
    private def createRequestMap() {
        ...
        new Requestmap(url: '/baseUser/**', configAttribute: 'ROLE_SUPERADMIN').save(flush: true)
